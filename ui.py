...
dnssec_success_text = _('DNSSEC Verfication for %(jid)s:\n' \
    '<span weight="bold" face="monospace">This fingerprint has been verified using DNSSEC</span>')
dnssec_unverified_text = _('DNSSEC Verfication for %(jid)s:\n' \
    '<span weight="bold" face="monospace">This fingerprint is present in the DNS record, but DNSSEC is not verified.</span>')
dnssec_failure_text = _('DNSSEC Verfication for %(jid)s:\n' \
    '<span weight="bold" face="monospace">The fingerprint verification using DNSSEC has failed.</span>')
...
class ContactOtrWindow(gtk.Dialog):
    ...
    def __init__(self, plugin, ctx, fpr=None, parent=None):
        ...
        if self.fpr is None:
            ...
            # show that the fingerprint is unknown
            self.gw('dnssec_label').set_markup(dnssec_failure_text % {
                    'jid': self.fjid})
            ...
            for (fpr, verified) in dnssec:
                if fpr == self.fpr.upper() and verified:
                    correct_fp = True
                    verified = True
                    break
                elif fpr == self.fpr.upper() and not verified:
                    correct_fp = True
                    break
            if correct_fp and verified:
                self.gw('dnssec_label').set_markup(dnssec_success_text % {
                    'jid': self.fjid})
            elif correct_fp:
                self.gw('dnssec_label').set_markup(dnssec_unverified_text % {
                    'jid': self.fjid})
            else:
                self.gw('dnssec_label').set_markup(dnssec_failure_text % {
                    'jid': self.fjid})
