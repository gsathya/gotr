...
class Context(object):
    ...
    def getCurrentDNSSECResult(self):
        return self.user.getDNSSECResult(self.trustName, fingerprint, default)
...
class Account(object):
    ...
    def setDNSSECResult(self, key, fpr, dnssecResult):
        if key not in self.dnssec:
            self.dnssec[key] = {}
        self.dnssec[key][fpr] = dnssecResult
    ...
    def getDNSSECResult(self, key, fingerprint):
        if key not in self.dnssec:
            return dnskeys.getOtrFingerprints(jid)
        else:
            return self.dnssec[key].get(fingerprint)
